    ...
    <div className="flex flex-col md:flex-row overflow-hidden relative flex-1 min-h-0">
      ...
      <div className="w-full md:w-1/2 border-r border-gray-200 dark:border-gray-700 flex flex-col min-h-0">
        <div className="p-6 flex flex-col flex-1 min-h-0">
          ...
          <TextareaWithLineNumbers className={`flex-1 w-full flex bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg shadow-inner text-gray-600 dark:text-gray-300 break-all focus-within:ring-2 ${isVerified ? 'focus-within:ring-green-500' : 'focus-within:ring-red-500'} focus-within:border-transparent font-mono text-base transition-all min-h-0 overflow-hidden ${tokenUpdating ? `ring-2 ${isVerified ? 'ring-green-500' : 'ring-red-500'} scale-[1.01]` : ''}`} gutterClassName="bg-gray-50 dark:bg-gray-900/50 text-gray-400 border-r border-gray-200 dark:border-gray-700 p-4 min-w-[2.5rem]" textareaClassName="bg-transparent p-4 border-none w-full h-full outline-none break-all" value={token} onChange={(e)=>handleTokenChange(e.target.value)} placeholder="Token will appear here..." spellCheck="false" />
